...
def execute(filters=None):
	...
	for rows in data: 
       		if order_count == 0: 
       			order_prev = rows[0] 
 			item_prev = rows[5]
			cust_group_prev = rows[13]
			customer_prev = rows[4]
			assigned_to_prev = rows[14]
			posting_date_prev = rows[1]
			del_date_prev = rows[2]
...
#				rows[7], rows[8], rows[9], rows[10], rows[11], item_pend_qty, per_qty
 #				]) 
			...
			order_work = rows[0]
                        item_work = rows[5]
			cust_group_work = rows[13]
			customer_work = rows[4]
			assigned_to_work = rows[14]
			posting_date_work = rows[1]
			del_date_work = rows[2]
			if rows[3] == temp_date:
				diff_days = 0
			else:
				diff_days = rows[3] - rows[2]
...
#				rows[7], rows[8], rows[9], rows[10], rows[11], item_pend_qty, per_qty
 #				]) 
				...
				summ_data.append([cust_group_prev, assigned_to_prev, order_prev, posting_date_prev, 
			 	del_date_prev, customer_prev, tot_si_qty, tot_del_qty, tot_pend_qty, per_qty
...
#				]) 
				...
				order_prev = order_work 
                                item_prev = item_work
				cust_group_prev = cust_group_work
				assigned_to_prev = assigned_to_work
				customer_prev = customer_work
				posting_date_prev = posting_date_work
				del_date_prev = del_date_work
	...
	summ_data.append([" ", " ", " ", " ", "Total Value and Percentage ", " ", full_tot_si_amt, full_tot_del_amt, 0, tot_per_amt
 				])
