...
def get_columns():
		...
		_("Item")+"::100",
		_("Cost Center")+"::100",
		_("Expense Account")+"::150",
		_("Income Account")+"::150"
...
#        else:
 #               frappe.throw(_("'To Date' is required"))
	...
	if filters.get("cost_center"):
                conditions += " and si.cost_center = '%s'" % frappe.db.escape(filters.get("cost_center"), percent=False)
...
def get_sales_details(filters):
        ...
        return frappe.db.sql("""select so.name as sales_invoice, si.cost_center, si.expense_account, si.income_account
                from `tabSales Invoice` so, `tabSales Invoice Item` si
                where so.name = si.parent and so.status != "Cancelled" order by so.name, si.item_code""" % conditions, as_dict=1)
